...
export const MicrositeModal: React.FC<MicrositeModalProps> = ({
  ...
  const handleOpenGiftModal = (gift: GiftItem) => {
    ...
    setContributionAmount(gift.targetPrice.toString());
    setReceiptFileName('');
  ...
  const finalizeGiftSubmission = (status: 'pendiente' | 'confirmado') => {
    const amountNum = Number(contributionAmount) || selectedGift?.targetPrice || 15000;
    
    if (selectedGift && onGiftContribute) {
      onGiftContribute(selectedGift.id, amountNum);
    }

    if (selectedGift && onAddReceivedGift) {
      onAddReceivedGift({
  ...
  return (
    <div className={containerClass}>
      <div className={cardClass}>
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      ...
                      <div>
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-gray-900 text-white hover:bg-black rounded-lg text-xs font-semibold transition-all cursor-pointer shadow-xs"
                        >
        ...
        {/* Modal for contributing to gift with dedications and payment flow */}
        {selectedGift && (
            ...
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 max-h-[92vh] overflow-y-auto animate-fade-in">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <h3 className="font-bold text-base text-gray-900">{selectedGift.title}</h3>
                </div>
                <button 
                ...
                <form onSubmit={handleContributeSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 border border-gray-300 rounded-lg text-sm font-bold text-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-gray-900 hover:bg-black text-white font-semibold rounded-xl text-xs sm:text-sm transition-all shadow-xs cursor-pointer disabled:opacity-50"
                  >
                    {isProcessingMp 
                      ? 'Confirmando con Mercado Pago...' 
                      : paymentMethod === 'transferencia'
                      ? `Confirmar regalo y comprobante (AR$ ${Number(contributionAmount).toLocaleString()})`
                      : `Confirmar con Mercado Pago (AR$ ${Number(contributionAmount).toLocaleString()})`}
                  </button>
                </form>
              )}
            </div>
